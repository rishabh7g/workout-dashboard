...
function openSwapSheet(todayKey) {
  ...
  if (!rows) rows = `<div style="padding:24px;text-align:center;color:var(--text2)">No upcoming days in schedule</div>`;
...
function doBorrow(todayKey, targetKey) {
  const b = loadBorrows(); b[todayKey] = targetKey; saveBorrows(b);
  closeSwapSheet(); render();
}
function undoBorrow(key) {
  const b = loadBorrows(); delete b[key]; saveBorrows(b); render();
}
...
function updateProgress(activeId) {
  ...
  for (const [sec, items] of Object.entries(groups)) {
    ...
    if (chk) chk.style.display = items.every(i => completedItems.has(i.id)) ? '' : 'none';
  }

  // Completion banner
  if (done === total && total > 0 && !document.getElementById('done-banner')) {
...
function resetProgress() {
  ...
  for (const item of allItems) {
    const el = document.querySelector(`[data-id="${item.id}"]`);
    if (el) el.className = 'item-card ' + (item.id === activeId ? 'active' : 'upcoming');
  }
  updateProgress(activeId);
...
function workoutContentHTML(workout) {
  ...
  let cur = null;
  for (const item of allItems) {
    if (!cur || cur.key !== item.section) { cur = { key: item.section, items: [] }; sections.push(cur); }
    cur.items.push(item);
  }
...
function render() {
  ...
  const workout = (WORKOUTS[entry.type] || RUNNING_DAYS[entry.type])?.[entry.variation];
  if (!workout) return;
